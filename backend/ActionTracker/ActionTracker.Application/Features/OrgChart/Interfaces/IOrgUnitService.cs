...
public interface IOrgUnitService
{
    ...
    Task<OrgUnitDto> CreateAsync(CreateOrgUnitRequestDto request, string userId, CancellationToken ct = default);

    /// <summary>Updates name, description, code, and parentId. Validates no circular reference.</summary>
    Task<OrgUnitDto> UpdateAsync(Guid id, UpdateOrgUnitRequestDto request, string userId, CancellationToken ct = default);
